...
%_eg_conditional_dropds(cobertur.assistidos_fatores);
proc sql;
	create table cobertur.assistidos_fatores as
	select t1.id_participante,
			...
			(case
				when t1.CdTipoBenefi = 4 and t1.IddPartiCalc < &MaiorIdad
					then (max(0, ((1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** (&MaiorIdad - t1.IddPartiCalc) - 1) / (((1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** ((&MaiorIdad - t1.IddPartiCalc) - 1)) * ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) - &Fb * (1 - (1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** - (&MaiorIdad - t1.IddPartiCalc))))
					else 0
			end) format=12.8 AS anpen,
			max(0, (1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** - (&MaiorIdad - t1.IddFilJovCalc)) format=12.8 as descap,
			(case
			...
			(case
				when t1.IddFilJovCalc is null
					then 0
					else (max(0, ((1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** (&MaiorIdad - t1.IddFilJovCalc) - 1) / (((1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** ((&MaiorIdad - t1.IddFilJovCalc) - 1)) * ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) - &Fb * (1 - (1 + ((1 + txc.taxa_juros) / (1 + &PrTxBenef) - 1)) ** - (&MaiorIdad - t1.IddFilJovCalc))))
...
%_eg_conditional_dropds(work.assistidos_fator_salario_tipo1);
proc sql;
	create table work.assistidos_fator_salario_tipo1 as
	select t1.id_participante,
			t2.reajuste_salario AS reajuste_salario
	from partic.ASSISTIDOS t1, premissa.REAJUSTE_SALARIAL t2
	where t1.CdPatrocPlan = t2.CdPatrocPlan 
	and t2.CD_TIPO_REAJUSTE_SALARIAL = 2 
	and t1.CdTipoFtAtuSal = 1
	order by t1.id_participante;
run;

%_eg_conditional_dropds(work.assistidos_fator_salario_tipo2);
proc sql;
	create table work.assistidos_fator_salario_tipo2 as
	select t1.id_participante, 
		   EXP(SUM(LOG(t2.COTVALOR))) format=.10 AS reajuste_salario
	from partic.assistidos t1, premissa.COTACAO t2
...
proc sql;
	update work.assistidos_fator_salario_tipo2 t1
	set reajuste_salario = 1
	...
	SELECT * FROM work.assistidos_fator_salario_tipo1
	OUTER UNION CORR 
	SELECT * FROM work.assistidos_fator_salario_tipo2
	order by id_participante;
quit;

data cobertur.assistidos_fatores;
	merge cobertur.assistidos_fatores work.assistidos_fator_salario;
	by id_participante;
run;
